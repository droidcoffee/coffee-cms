...
<div class="body-box">
	...
	<form action="query.jsp" method="post" style="padding-top:5px;">
		<div>
		用户名: <input type="text" name="uname" value="${uname}" style="width:100px"/>
			...
			<tbody class="pn-ltbody">
				<c:forEach items="${pager.items}" var="item">
					<tr onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
						...
						<td>${item.uname}</td>
						<% 
							UserBean bean = (UserBean)pageContext.findAttribute("item");
							pageContext.setAttribute("addTime", new Date(bean.getAddTime()));
							pageContext.setAttribute("lastLoginTime", new Date(bean.getLastLoginTime()));
						%>
						<td><fmt:formatDate value="${addTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
						<td><fmt:formatDate value="${lastLoginTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
						<td align="center">		
